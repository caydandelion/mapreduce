   ...
   // part3-----------------------------------------------------
    ...
    public static class Reduce_Part3 extends Reducer<Text, Text, Text, Text> {
        ...
        public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
            ...
            int i = 0;
            for (Text str : values) {
            	String[] itr = str.toString().split(" ");
                words.add(itr[0]); // 保存
                frequence.add(itr[1]);
            }
            String t = new String();
            LoadFeature wo = new LoadFeature();
            List<String> lis = null;
            lis = wo.loadfe();
            for (int j = 0; j < lis.size(); j++) {
                ...
                for(int c = 0;c < words.size();c++){
                	if(val.equals(words.get(c)))
                	{
                		count = frequence.get(c);
                		bool = 1;
                		break;
                	}
                }
                if(bool == 1 && j != lis.size()-1)
                   t = t +count+",";
                else if(bool == 1 && j == lis.size()-1)
                	t = t +count;
                else if(bool == 0 && j == lis.size()-1)
                	t = t +"0";
                else
                   t = t +"0"+",";
            }
            String positive ="1";
            context.write(new Text(positive), new Text(t));
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        job1.setJarByClass(TextVector.class);
        ...
        job2.setJarByClass(TextVector.class);
